...
void TexRectToN64FrameBuffer_16b(uint32 x0, uint32 y0, uint32 width, uint32 height, uint32 dwTile)
{
	...
	sprintf(txtbuffer,"TexRectToN64FrameBuffer_16b");
...
#if 0
		sprintf(txtbuffer,"CalculateRDRAMCRC: fast dwAsmCRC %xd", dwAsmCRC);
...
#endif
#if 0
		sprintf(txtbuffer,"CalculateRDRAMCRC: slow dwAsmCRC %xd", dwAsmCRC);
		DEBUG_print(txtbuffer,DBG_CACHEINFO+2); 
#endif
        }
        catch(...)
        {
...
{
#if 0 //temporarily disable
	sprintf(txtbuffer,"UpdateFrameBufferBeforeUpdateFrame");
	DEBUG_print(txtbuffer,DBG_RICE+3); 
	return;
#endif
...
{
#if 0 //temporarily disable
	sprintf(txtbuffer,"SetBackBufferAsRenderTexture");
	DEBUG_print(txtbuffer,DBG_RICE+3); 
	return -1;
#endif
/* MUDLORD:
...
#else //!__GX__
        new CGXRenderTexture(tempRenderTextureInfo.bufferWidth, tempRenderTextureInfo.bufferHeight, &gRenderTextureInfos[idxToUse], AS_BACK_BUFFER_SAVE);
#endif //__GX__
    }
...
{
#if 0 //temporarily disable
	sprintf(txtbuffer,"LoadTextureFromRenderTexture");
	DEBUG_print(txtbuffer,DBG_RICE+3); 
	return;
#endif
...
{
#if 0 //temporarily disable
	sprintf(txtbuffer,"ActiveTextureBuffer");
    ...
    if( status.bHandleN64RenderTexture )
    {
...
#else //!__GX__
                new CGXRenderTexture(w, newRenderTextureInfo.bufferHeight, &gRenderTextureInfos[idxToUse], AS_RENDER_TARGET);
#endif //__GX__
        }
        ...
        if( gRenderTextureInfos[idxToUse].pRenderTexture->SetAsRenderTarget(true) )
        {
			...
			//The following were previously removed with DX stuff
...
{
#if 0 //temporarily disable
	sprintf(txtbuffer,"CopyBufferToRDRAM");
	DEBUG_print(txtbuffer,DBG_RICE+3); 
	return;
#endif
    uint32 startline=0;
...
{
#if 0 //temporarily disable
	sprintf(txtbuffer,"SaveBackBuffer");
	DEBUG_print(txtbuffer,DBG_RICE+3); 
	return;
#endif
