...
void TLB_refill_exception(unsigned long address, int w)
{
	...
	{ // Test de EXL
		...
		{ //wii64: path will never be taken (that's ok)
			jump_to(0x80000180);
		}
	...
	else
	{
		...
		{
			if (r4300emu || interpcore)
			{ //wii64: used to be just interpcore in mupen64 0.5 code
				interp_addr = 0x80000180;
			}
			else
			{ //wii64: path never taken
				jump_to(0x80000180);
			}
		}
		else
	...
	if (!r4300emu || dyna_interp) {
	  dyna_interp = 0;
	 */
	if (delay_slot)
	{
		if (r4300emu || interpcore)
		{ //wii64: used to be (!r4300emu || interpcore) in mupen64 0.5 code
...
void exception_general()
{
	...
	if (!r4300emu && !interpcore)
	{ // wii64: used to be !interpcore in mupen64 0.5 code
		EPC = PC->addr;
	}
	else
	{
