...
void gen_interupt()
{
	...
	if (skip_jump)
	{
		...
		{ // wii64: originally this was for interpcore in mupen 0.5 (wii64 changed it)
			interp_addr = skip_jump;
			last_addr = interp_addr;
		}
		else
		{ // wii64: This path will never never hit?
	...
	case AI_INT:
		if (ai_register.ai_status & 0x80000000)
		{ // full
	...
	case SP_INT:
		...
		if (!(sp_register.sp_status_reg & 0x40))
		{
			return;
		}
