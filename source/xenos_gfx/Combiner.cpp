...
{
    xeComb_updateColors((TxeCombiner*)combiner.current->compiled);
...
CachedCombiner *Combiner_Compile( u64 mux )
{
    ...
    cached->compiled = xeComb_compile(&color,&alpha);
...
{
    xeComb_beginTextureUpdate();
}

void Combiner_EndTextureUpdate()
{
    xeComb_endTextureUpdate();        
}
...
{
    xeComb_setCombiner((TxeCombiner*)combiner.current->compiled );
}
