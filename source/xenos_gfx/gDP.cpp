...
void gDPFillRectangle( s32 ulx, s32 uly, s32 lrx, s32 lry )
{
	...
	if (gDP.otherMode.cycleType == G_CYC_FILL)
	{
		...
		{
            xeGfx_clearColorBuffer( &gDP.fillColor.r );
			return;
		}
	}

    xeGfx_drawRect( ulx, uly, lrx, lry, (gDP.otherMode.cycleType == G_CYC_FILL) ? &gDP.fillColor.r : &gDP.blendColor.r );
...
void gDPTextureRectangle( f32 ulx, f32 uly, f32 lrx, f32 lry, s32 tile, f32 s, f32 t, f32 dsdx, f32 dtdy )
{
	...
	{
		if (lrt > t)
			xeGfx_drawTexturedRect( ulx, uly, lrx, lry, s, t, lrs, lrt, (RSP.cmd == G_TEXRECTFLIP) );
		else
			xeGfx_drawTexturedRect( ulx, lry, lrx, uly, s, lrt, lrs, t, (RSP.cmd == G_TEXRECTFLIP) );
	}
	else
	{
		if (lrt > t)
			xeGfx_drawTexturedRect( lrx, uly, ulx, lry, lrs, t, s, lrt, (RSP.cmd == G_TEXRECTFLIP) );
		else
			xeGfx_drawTexturedRect( lrx, lry, ulx, uly, lrs, lrt, s, t, (RSP.cmd == G_TEXRECTFLIP) );
	}
