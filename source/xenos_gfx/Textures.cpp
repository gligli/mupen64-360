...
inline u32 FixTexel(u32 t){
    return (t<<24) | ( (t>>8) & 0xffffff );
}
...
{ //		Get32					
	{ // 4-bit
		{	GetCI4RGBA_RGBA8888,	4, 4096 }, // CI (Banjo-Kazooie uses this, doesn't make sense, but it works...)
		{	GetNone,				4, 8192 }, // YUV
		{	GetCI4RGBA_RGBA8888,	4, 4096 }, // CI
		{	GetIA31_RGBA8888,		4, 8192 }, // IA
		{	GetI4_RGBA8888,			4, 8192 }, // I
	},
	{ // 8-bit
		{	GetCI8RGBA_RGBA8888,	3, 2048 }, // RGBA
		{	GetNone,				0, 4096 }, // YUV
		{	GetCI8RGBA_RGBA8888,	3, 2048 }, // CI
		{	GetIA44_RGBA8888,		3, 4096 }, // IA
		{	GetI8_RGBA8888,			3, 4096 }, // I
	},
	{ // 16-bit
		{	GetRGBA5551_RGBA8888,	2, 2048 }, // RGBA
		{	GetNone,				2, 2048 }, // YUV
		{	GetNone,				0, 2048 }, // CI
		{	GetIA88_RGBA8888,		2, 2048 }, // IA
		{	GetNone,				0, 2048 }, // I
	},
	{ // 32-bit
		{	GetRGBA8888_RGBA8888,	2, 1024 }, // RGBA
		{	GetNone,				0, 1024 }, // YUV
		{	GetNone,				0, 1024 }, // CI
		{	GetNone,				0, 1024 }, // IA
		{	GetNone,				0, 1024 }, // I
...
{
	u32 dummyTexture[4] = { 0, 0xffffffff, 0xffffffff, 0};
...
void TextureCache_LoadBackground( CachedTexture *texInfo )
{
	...
	for (y = 0; y < texInfo->realHeight; y++)
	{
		...
		for (x = 0; x < texInfo->realWidth; x++)
		{
			...
			((u32*)dest)[j++] = FixTexel(GetTexel( (u64*)src, tx, 0, texInfo->palette ));
		}
	}
	...
	if (cache.enable2xSaI)
	{
        ...
        texInfo->xeSurface=xeGfx_createTexture(texInfo->realWidth << 1,texInfo->realHeight << 1);
        xeGfx_setTextureData(texInfo->xeSurface,scaledDest);
	...
	{
        texInfo->xeSurface=xeGfx_createTexture(texInfo->realWidth,texInfo->realHeight);
        xeGfx_setTextureData(texInfo->xeSurface,dest);
		free( dest );
	}
}
...
void TextureCache_Load( CachedTexture *texInfo )
{
	...
	for (y = 0; y < texInfo->realHeight; y++)
	{
		...
		for (x = 0; x < texInfo->realWidth; x++)
		{
            ...
            ((u32*)dest)[j++] = FixTexel(GetTexel( src, tx, i, texInfo->palette));
		}
	}
...
{
    xeGfx_activateTexture(texture->xeSurface,t,(gDP.otherMode.textureFilter == G_TF_BILERP) || (gDP.otherMode.textureFilter == G_TF_AVERAGE),texture->clampS,texture->clampT);
...
//TextureCache_ActivateTexture( t, cache.dummy );
    ...
    xeGfx_activateTexture(cache.dummy->xeSurface,t,1,0,0);
}
...
void TextureCache_Update( u32 t )
{
	...
	{
        xeGfx_activateFrameBufferTexture(t);
		return;
	}
...
/*	if (cache.current[t]->clampS)
	...
	cache.current[t]->offsetS = cache.enable2xSaI ? 0.25f : 0.5f;
	cache.current[t]->offsetT = cache.enable2xSaI ? 0.25f : 0.5f;
